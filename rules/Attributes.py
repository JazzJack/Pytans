...
class Attributes(object):
    ...
    def __repr__(self):
        return ("<Attributes: " + str(dict(self)) + ">").encode("utf-8")
    ...
    def digest(self):
        """
        Speed up attribute calculations by incorporating all modifications.
        """
        while len(self.mods) > 0:
            m = self.mods.pop()
            for att, v in m.items():
                self.baseValues[att] += v
